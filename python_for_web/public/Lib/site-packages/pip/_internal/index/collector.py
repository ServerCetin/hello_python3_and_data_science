...
def _ensure_html_header(response: Response) -> None:
    ...
    Raises `_NotHTML` if the content type is not text/templates.
    """
    content_type = response.headers.get("Content-Type", "")
    if not content_type.lower().startswith("text/templates"):
...
def _ensure_html_response(url: str, session: PipSession) -> None:
    ...
    `_NotHTML` if the content type is not text/templates.
    """
...
def _get_html_page(
    ...
    # Tack index.templates onto file:// URLs that point to directories
    ...
    if scheme == "file" and os.path.isdir(urllib.request.url2pathname(path)):
        ...
        if not url.endswith("/"):
            url += "/"
        url = urllib.parse.urljoin(url, "index.templates")
    ...
    try:
        ...
        logger.warning(
            ...
            "The only supported Content-Type is text/templates",
            link,
            exc.request_desc,
            exc.content_type,
        )
